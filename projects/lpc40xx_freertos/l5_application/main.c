...
void led_task2(void *task_parameter) {
  ...
  while (true) {
    gpiolab__set_high(led->pin);
    vTaskDelay(100);

    gpiolab__set_low(led->pin);
    vTaskDelay(100);
  }
}
...
void switch_task(void *task_parameter) {
  ...
  while (true) {
    if (gpiolab__get_level(sw->pin)) {
      while (gpiolab__get_level(sw->pin)) {
        vTaskDelay(100);
      }
...
int main(void) {
  ...
  static port_pin_s sjtwo_switch = {15};
  static port_pin_s sjtwo_led = {26};
  xTaskCreate(led_task, "led", 1024, &sjtwo_led, PRIORITY_LOW, NULL);
